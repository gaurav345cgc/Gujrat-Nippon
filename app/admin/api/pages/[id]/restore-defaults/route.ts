...
import { getPageById, restorePageDefaults } from '@/lib/cms/service';
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const snapshot = await restorePageDefaults(id, slug, session.user.id);

    await writeAuditLog({
      actorId: session.user.id,
      action: 'cms.page.restore_defaults',
      entityType: 'page',
      entityId: id,
      metadata: { slug },
      ip,
    ...
    return jsonOk({ page: snapshot, editor: getPageEditorDefaults(slug) });
  } catch (e) {
    const message = e instanceof Error ? e.message : 'Restore defaults failed.';
